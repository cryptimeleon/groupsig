...
public class CPY06XiProof extends DelegateProtocol {
    ...
    protected SubprotocolSpec provideSubprotocolSpec(CommonInput commonInput, SubprotocolSpecBuilder builder) {
        ...
        SchnorrZnVariable xVar = builder.addZnVariable(
                "x",
                xiProofCommonInput.getPp().getBilGroup().getG1().getZn()
        );
        ...
        GroupEqualityExpr xStatement = xiProofCommonInput.getPi().expr()
                .isEqualTo(xiProofCommonInput.getPp().getP1().pow(xVar));

        // v P + u I - P_i = r' H
        // TODO: Not sure what H from [NguSaf04] corresponds to in [CYP06]. Only option seems to be Z.
        GroupEqualityExpr rPrimeStatement = xiProofCommonInput.getPp().getP1().pow(xiProofCommonInput.getV())
                .op(xiProofCommonInput.getI().pow(xiProofCommonInput.getU()))
                .op(xiProofCommonInput.getPi().inv())
                .expr().isEqualTo(xiProofCommonInput.getPp().getZ().pow(rPrimeVar));
